function results = DDAD_sharp_layer_study(cfg)
%DDAD_SHARP_LAYER_STUDY
%
% Global distribution:
%   DDAD.
%
...
%
% No independently generated solution-data locations are used.
% Both global DDAD and growth DDAD sample existing PDE interior points.
    ...
    ad_train = ...
        resolve_ddad_training(cfg,problem);
    ...
    Xdata_ad = ...
        problem.Xi(ad_train.indices,:);

    noise_unit_ad = ...
        make_fixed_noise(size(Xdata_ad,1),cfg.seed);

    fprintf('\n');
    fprintf('============================================================================\n');
    fprintf('DDAD + DDAD LAYER GROWTH | NEWTON SOLVER\n');
    fprintf('============================================================================\n');
    fprintf('seed                            = %d\n',cfg.seed);
    fprintf('full m1 / m2                    = %d / %d\n',cfg.m1,cfg.m2);
    fprintf('all interior points             = %d\n',size(problem.Xi,1));
    fprintf('\n');
    fprintf('SWITCH 1: global DDAD reduction = %d\n',ad_train.enabled);
    fprintf('  DDAD train m1                  = %d / %d\n',ad_train.m1,cfg.m1);
    fprintf('  DDAD train points              = %d / %d\n', ...
        ad_train.num_points,size(problem.Xi,1));
    fprintf('\n');
    ...
    fprintf('DDAD target source              = current numerical solution\n');
    fprintf('exact data used in training     = 0\n');
    fprintf('============================================================================\n\n');
    ...
    base = ...
        solve_ddad_newton( ...
            problem,basis,basis_ad_train, ...
            Xdata_ad,noise_unit_ad,ad_train,cfg);
    ...
    fprintf('\n');
    fprintf('================================================================================\n');
    fprintf('FINAL REPORT: DDAD VS. DDAD + LAYER GROWTH\n');
    fprintf('================================================================================\n');
    ...
    ADTrainPoints = [ad_train.num_points;ad_train.num_points];
    ...
    summary = table( ...
        Stage,m1,m2, ...
        ADReduction,ADTrainM1,ADTrainPoints, ...
    ...
    writetable( ...
        summary, ...
        fullfile(out_dir,'DDAD_sharp_layer_summary.csv'));
    ...
    results.target_source = 'current_numerical_solution_on_problem_Xi';
    ...
    save( ...
        fullfile(out_dir,'DDAD_sharp_layer_newton_results.mat'), ...
        'results','-v7.3');
end


function ad_train = resolve_ddad_training(cfg,problem)
    ...
    required = {'enabled','m1_train','num_points'};
    ...
    if ad_train.enabled
        ...
        ad_train.num_points = s.num_points;
    else
        ad_train.m1 = cfg.m1;
        ad_train.num_points = Nfull;
    end

    validate_count(ad_train.m1,cfg.m1,'DDAD m1_train','m1');
    validate_count(ad_train.num_points,Nfull,'DDAD num_points','all interior points');

    ad_train.indices = ...
        select_training_subset_indices( ...
            Nfull,ad_train.num_points,cfg.seed);
end
...
function base = solve_ddad_newton( ...
    problem,basis,basis_ad_train, ...
    Xdata,noise_unit,ad_train,cfg)
    ...
    for it = 1:cfg.newton.maxit
        ...
        if should_adapt
            ...
            fprintf('\n');
            fprintf('>>> COMPLETE DDAD UPDATE %d/%d at Newton iteration %d\n', ...
                update_id,cfg.ad.max_updates,it);
            fprintf('    p before          = [%.8f, %.8f]\n',p(1),p(2));
            fprintf('    AD train features = %d / solve m1=%d\n', ...
                ad_train.m1,cfg.m1);
            fprintf('    AD train points   = %d / all Xi=%d\n', ...
                ad_train.num_points,size(problem.Xi,1));
            fprintf('    inner Adam maxit  = %d\n',cfg.ad.optimizer.maxit);

            p_before = p;

            % Target = CURRENT full-m1 numerical solution evaluated on
            % existing PDE interior points selected by switch 1.
            target_state = ...
                evaluate_gaussian_state( ...
                    Xdata,p,basis,coef,0,cfg.chunk_rows);
            ...
            cache_data = ...
                prepare_data_cache( ...
                    Xdata,y_target,basis_ad_train);

            fprintf('    target            = current numerical solution\n');
            fprintf('    target noise      = %.3e\n',cfg.data.noise_delta);

            objective_fun = @(pp) ...
                evaluate_data_reduced_fast( ...
                    pp,cache_data,cfg.ad.lambda, ...
            ...
            ad_log(update_id).target_norm = norm(y_target);
            ad_log(update_id).target_noise_delta = cfg.data.noise_delta;
...
function [coef_new,alpha,res_l2,res_mse,accepted] = ...
    ...
    if newton_cfg.line_search
        ...
        for ls_it = 1:newton_cfg.line_search_maxit
            ...
            [~,~,~,~,res_l2,res_mse] = ...
                nonlinear_residual(coef_trial,op,problem);
