...
MATLAB R2022b or later is recommended. No Python environment is required for the MATLAB examples. Before public release, the exact tested MATLAB version and any required toolboxes should be recorded here.
...
```matlab
run("experiments/poisson/run_seed_study.m")
```

Additional experiments are available under `examples/`, including Allen-Cahn, Burgers, Helmholtz, Black-Scholes, L-shaped Poisson problems, and operator-learning examples. Use the corresponding `run_*.m` script as the entry point and edit the nearby `config*.m` file when necessary.
...
- `examples/`: numerical examples and configuration files
- `experiments/`: result-reproduction scripts
...
A `BSD-3-Clause` license file should be added before the repository is made public.
