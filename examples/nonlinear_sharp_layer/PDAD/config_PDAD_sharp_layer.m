function [cfg,project_root] = config_PDAD_sharp_layer()
%CONFIG_PDAD_SHARP_LAYER
%
%   cfg = config_PDAD_sharp_layer()
...
%       DDAD/
%
% This configuration activates only the PDAD subtree and removes the DDAD
...
%      Controls only the global PDAD training of p:
...
% All Newton PDE solves always use the full cfg.m1 and cfg.m2 spaces.
    ...
    % =====================================================================
    % Locate project root and activate only the PDAD implementation
    % =====================================================================
    project_root = initialize_sharp_layer_project('PDAD');

    % =====================================================================
    % Load the existing method-specific base configuration
    %
    % Because the PDAD subtree has been placed at the beginning of the
    % MATLAB path, config() resolves to the PDAD-side config.m.
    % =====================================================================
    if isempty(which('config'))
        error('config_PDAD_sharp_layer:MissingBaseConfig', ...
            ['Could not find config.m after activating the PDAD source ', ...
             'directory.']);
    end
    ...
    if ~isstruct(cfg)
        error('config_PDAD_sharp_layer:InvalidBaseConfig', ...
    ...
    % =====================================================================
    % Switch 1: global PDAD training reduction
    %
    % false:
    ...
    % PDAD uses both interior PDE equations and boundary equations.
    cfg.ad_training_reduction.num_interior_points = 3000;
    cfg.ad_training_reduction.num_boundary_points = 200;
    ...
    % Number of physical rows processed in one feature/gradient chunk.
    cfg.chunk_rows = 500;
    ...
    % Growth DDAD samples existing problem.Xi points.
    % =====================================================================
    ...
    % The growth evaluator minimizes the ridge-reduced objective, so the
    % selected checkpoint is based on the same objective.
...
%   upper_bound = 300
%
% Scalar bounds are automatically expanded by
% optimize_distribution_adam.m for vector-valued parameters.
...
%
% For method='PDAD':
%   - PDAD/ is added at the beginning of the path;
%   - DDAD/ is removed from the path.
%
% For method='DDAD':
%   - DDAD/ is added at the beginning of the path;
%   - PDAD/ is removed from the path.
    ...
    if ~isempty(cached_project_root) && ...
       exist(cached_project_root,'dir') == 7
        ...
        if isempty(project_root)
            ...
            error('config_PDAD_sharp_layer:ProjectRootNotFound', ...
                [ ...
    ...
    % Add all common source modules first.
    addpath(genpath(src_dir));

    % Remove both method-specific trees to eliminate stale path ordering.
    ...
    % The currently requested method must have highest path priority.
